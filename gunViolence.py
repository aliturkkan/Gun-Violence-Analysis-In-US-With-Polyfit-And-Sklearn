...
data = read_csv('gun-violence-data.csv')
length = len(data)
...
try:
    estimation = float(input("Want to know which month's data estimate:\t"))
    ...
    if estimation >= 1 and estimation <= 12:
        ...
        for i in range(len(gun_violence_count_by_days)):
            error = error + (
            gun_violence_count_by_days[i] - (a * (z[i] ** 7) + b * (z[i] ** 6) + c * (z[i] ** 5) + d * (z[i] ** 4) +
        ...
        plot(z, a * (z ** 7) + b * (z ** 6) + c * (z ** 5) + d * (z ** 4) + e * (z ** 3) + f * (z ** 2) + g * z + h,
             color="g")
        show()
    else:
